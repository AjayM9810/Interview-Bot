...
BANNED_DISPLAY_MESSAGE = (
    "Your are banned for Violating our policies, So you are banned from attending interviews for 1 year"
)
VOICE_MANIPULATION_BAN_MESSAGE = (
    "You are banned, Violated the rules and Tired to manipulate the results"
)
VOICE_MISMATCH_DISTANCE_THRESHOLD = 0.18
...
def extract_voice_signature(audio_bytes):
    if not audio_bytes:
        return None
    ...
    if sample_rate <= 0 or frame_count <= 0 or not raw_frames:
        return None
    ...
    signal = np.frombuffer(raw_frames, dtype=dtype).astype(np.float32)
    if signal.size == 0:
        return None
    ...
    target_samples = int(min(signal.size, sample_rate * 3.0))
    start = max(0, (signal.size - target_samples) // 2)
    segment = signal[start:start + target_samples]
    if segment.size < 512:
        return None

    segment = segment - np.mean(segment)
    energy = np.abs(segment)
    non_silent = segment[energy > np.percentile(energy, 35)]
    if non_silent.size >= 512:
        segment = non_silent

    if segment.size < 512:
        return None

    sign_changes = np.mean(np.abs(np.diff(np.signbit(segment).astype(np.int8))))
    zcr = float(sign_changes)
    ...
    upper_freq = min(sample_rate / 2.0, 8000.0)
    band_edges = np.linspace(0.0, upper_freq, 9)
    band_features = []
    for left, right in zip(band_edges[:-1], band_edges[1:]):
        mask = (freqs >= left) & (freqs < right)
        band_features.append(float(np.log1p(np.sum(spectrum[mask]))))

    band_sum = sum(band_features) or 1.0
    band_features = [value / band_sum for value in band_features]

    vector = np.array(
        [
            zcr,
            centroid / max(upper_freq, 1.0),
            rolloff / max(upper_freq, 1.0),
            *band_features,
        ],
        dtype=np.float32,
    )
...
def voice_distance(signature_a, signature_b):
    if not signature_a or not signature_b:
        return 0.0

    a = np.array(signature_a.get("vector", []), dtype=np.float32)
    b = np.array(signature_b.get("vector", []), dtype=np.float32)
...
def summarize_policy_violations(answer_rows):
    flagged = [row for row in answer_rows if bool(row.get("ai_flag")) or bool(row.get("plagiarism_flag"))]
    ...
    return {
        ...
        "voice_mismatch_questions": [],
    }


def ban_candidate(username, reason):
...
def get_ban_display_message(ban_reason):
    reason = _normalize_text(ban_reason)
    if "manipulate the results" in reason or "voice mismatch" in reason:
        return VOICE_MANIPULATION_BAN_MESSAGE
    return BANNED_DISPLAY_MESSAGE
    ...
    for question_index, question in enumerate(questions):
        ...
        if answer_text:
            ...
            st.session_state.answers[display_question_text] = {
                ...
                "violation_flags": policy["violation_flags"],
                "violation_remark": policy["violation_remark"],
                "audio_present": bool(st.session_state.answer_audio_blobs.get(question_index)),
            }
        else:
...
def submit_interview_now(username, role, experience, questions):
    ...
    policy_summary = summarize_policy_violations(answer_payload)

    if policy_summary["ban_candidate"]:
        ban_details = ban_candidate(username, policy_summary["ban_reason"])
        results["banned"] = True
        results["ban_until"] = ban_details["ban_until"]
        results["ban_reason"] = ban_details["ban_reason"]
        results["ban_remark"] = policy_summary.get("ban_message") or get_ban_display_message(
            ban_details["ban_reason"]
        )
    else:
        results["banned"] = False

    results["policy_flagged_answers"] = policy_summary["flagged_count"]
    results["voice_mismatch_questions"] = policy_summary.get("voice_mismatch_questions", [])
...
""",
    ...
    if st.session_state.interview_submitted and st.session_state.results_summary:
        ...
        if summary.get("banned"):
            st.warning("Interview submission is under policy review. Our team will contact you soon.")
        else:
            score = get_display_score(summary)
            st.success("Interview complete!")
            st.info(f"Your score: {score} / {total_questions}")
...
<div class="summary-card">
  ...
  <div class="summary-grid">
    <div class="summary-item"><div class="summary-label">Candidate</div><div class="summary-value">{username}</div></div>
...
""",
    ...
    with st.container(border=True):
        ...
        st.caption("Interview security mode is active: copy, paste, cut, and right-click are disabled.")
