...
def get_dashboard_data():
    query = """
    SELECT cp.username, cp.email, cp.phone, cp.experience, cp.previous_role,
...
def get_latest_candidate_profile_snapshot(username):
    row = conn.execute(
        """
        SELECT phone, email, experience, previous_role
        FROM candidate_profiles
        WHERE username = ?
        ORDER BY id DESC
        LIMIT 1
    ...
    return {
        "phone": str(row[0] or "Not provided").strip() or "Not provided",
        "email": str(row[1] or "Not provided").strip() or "Not provided",
        "experience": str(row[2] or "Not provided").strip() or "Not provided",
        "previous_role": str(row[3] or "Not provided").strip() or "Not provided",
    }
...
def build_admin_candidate_report_pdf(username, role, feedback, answer_rows, ban_status):
...
def show_answers_and_insights(username):
    ...
    with st.spinner("Loading candidate insights..."):
        ...
        report_pdf = build_admin_candidate_report_pdf(username, candidate_role, feedback, answers, ban_status)

    status_text = "Banned" if ban_status else "Active"
    status_css = "banned" if ban_status else "active"
    avatar_text = _escape_html((str(username or "?").strip()[:1] or "?").upper())
...
<div class="summary-card">
  ...
  <div class="summary-grid">
    <div class="summary-item"><div class="summary-label">Name</div><div class="summary-value">{_escape_html(username)}</div></div>
    ...
    with tab_overview:
        ...
        if feedback:
            ...
            if feedback["topic_analysis"]:
                ...
                st.dataframe(pd.DataFrame(feedback["topic_analysis"]), width="stretch")
...
.main {
    background:
      radial-gradient(circle at 8% 15%, rgba(24, 169, 153, 0.08) 0%, rgba(24, 169, 153, 0) 36%),
...
html[data-theme="dark"] .main {
    background:
      radial-gradient(circle at 8% 15%, rgba(24, 169, 153, 0.18) 0%, rgba(24, 169, 153, 0) 34%),
      radial-gradient(circle at 86% 0%, rgba(11, 77, 182, 0.22) 0%, rgba(11, 77, 182, 0) 38%),
...
if not st.session_state.admin_logged_in:
    ...
    if login_wallpaper_uri:
        login_background_css = (
            "background: linear-gradient(135deg, rgba(18, 40, 70, 0.38) 0%, rgba(23, 52, 86, 0.42) 55%, "
            f"rgba(19, 43, 73, 0.46) 100%), url('{login_wallpaper_uri}') center center / cover no-repeat !important;"
        )
    else:
        login_background_css = (
...
div[data-testid="stTextInput"] > div > div {
    ...
    box-shadow: inset 0 1px 0 rgba(255,255,255,0.65), 0 6px 14px rgba(44, 77, 122, 0.12);
}
div[data-testid="stTextInput"] > div > div:focus-within {
    border-color: #3f79c9;
    box-shadow: 0 0 0 2px rgba(63, 121, 201, 0.22), 0 10px 18px rgba(44, 77, 122, 0.14);
}
...
div[data-testid="stButton"] > button {
    ...
    display: block;
    border-radius: 12px;
    background: linear-gradient(180deg, #2f79dd 0%, #1f62c8 100%);
    border: 1px solid #2e73d3;
    color: #ffffff;
    font-size: 20px;
    font-weight: 700;
    letter-spacing: 0.01em;
    box-shadow: 0 8px 16px rgba(20, 84, 173, 0.24);
}
div[data-testid="stButton"] > button:hover {
    background: linear-gradient(180deg, #3a84e8 0%, #276dd0 100%);
}
</style>
...
<div class="admin-login-shell">
  <div class="admin-login-logo">ABC<span class="inc">INC</span></div>
  <div class="admin-login-subtitle">Welcome to ABC Control Center</div>
  <div class="admin-login-note">Secure Administrative Access</div>
</div>
""",
        unsafe_allow_html=True,
    )

    _, login_col, _ = st.columns([1.3, 2.4, 1.3])
    with login_col:
        ...
        if st.button("Login", width="stretch", key="admin_login_button"):
...
""",
    ...
    if df.empty:
        ...
        with st.container(border=True):
            ...
            with filter_col1:
                search_text = st.text_input("Search Candidate", placeholder="Type username/email")
            with filter_col2:
        ...
        with st.container(border=True):
            ...
            if not candidate_options:
                ...
                selected_candidate = st.selectbox("Candidate Username", candidate_options, index=selected_index)
        ...
        if show_analytics:
            with st.container(border=True):
                ...
                with vis_col1:
                    st.markdown("**Average Score by Role (Bar)**")
                ...
                with vis_col2:
                    st.markdown("**Hard Question Success Rate by Role (Line)**")
                    if hard_success_df.empty:
                        ...
                        render_fixed_line_chart(
                            ...
                            "Role",
                            ["Hard Success Rate"],
                            "Hard Question Success Rate by Role",
                            color_scale={"domain": ["Hard Success Rate"], "range": ["#2e7d32"]},
                        )

                st.markdown("**Result Distribution (Pie)**")
            ...
            with st.container(border=True):
                ...
                if not role_options:
                    ...
                    if len(role_candidates) < 2:
                        ...
                        with comp_col1:
                            candidate_a = st.selectbox("Candidate A", role_candidates, key="compare_candidate_a")
                        with comp_col2:
                            candidate_b = st.selectbox("Candidate B", role_candidates, key="compare_candidate_b", index=1 if len(role_candidates) > 1 else 0)
...
<div class="compare-card">
  ...
  <div class="compare-card-value">{_escape_html(str(candidate_a))} | Score {int(_safe_float(card_a.get('Score'), 0))}/15</div>
</div>
""",
                                unsafe_allow_html=True,
                            )
...
<div class="compare-card">
  ...
  <div class="compare-card-value">{_escape_html(str(candidate_b))} | Score {int(_safe_float(card_b.get('Score'), 0))}/15</div>
</div>
""",
                                unsafe_allow_html=True,
                            )
